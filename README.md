...
这书坑挺多的，萌新入门都不推荐看这本书(代码打印的版式有些问题)，班上一些基础较弱的同学写这本书上的代码根本没法独立完成。不过有一定基础的同学看的话还是勉强作为入门教程的。书上的章节安排也算合理。该有的东西也还都有。

这本书的代码已经不太适合目前的开发情况了，书上的内容有些跟不上时代，里面的代码可能不嫩在现代开发环境下很好的运行，所以这里将书上所有的代码示例整合进一个app，方便大家学习的时候有个参考。同时也将可能遇到的错误记录在这份文件中自己学也是ok的。
